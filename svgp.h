 ...
 */
  ...
  switch (str_hash)
  {
    ...
    case 'K': *args = svgp->color_components; return CTX_SET_COLOR;
...
static void svgp_dispatch_command (SvgP *svgp)
{
  ...
  switch (cmd)
  {
    ...
    case CTX_STROKE_TEXT: ctx_text_stroke (ctx, (char*)svgp->holding); break;
    case CTX_VER_LINE_TO: ctx_line_to (ctx, ctx_x (ctx), svgp->numbers[0]); svgp->command = CTX_VER_LINE_TO;
	svgp->pcx = ctx_x (ctx);
	svgp->pcy = ctx_y (ctx);
        break;
    case CTX_HOR_LINE_TO:
	ctx_line_to (ctx, svgp->numbers[0], ctx_y(ctx)); svgp->command = CTX_HOR_LINE_TO;
	svgp->pcx = ctx_x (ctx);
	svgp->pcy = ctx_y (ctx);
	break;
    case CTX_REL_HOR_LINE_TO: ctx_rel_line_to (ctx, svgp->numbers[0], 0.0f); svgp->command = CTX_REL_HOR_LINE_TO;
	svgp->pcx = ctx_x (ctx);
	svgp->pcy = ctx_y (ctx);
        break;
    case CTX_REL_VER_LINE_TO: ctx_rel_line_to (ctx, 0.0f, svgp->numbers[0]); svgp->command = CTX_REL_VER_LINE_TO;
	svgp->pcx = ctx_x (ctx);
	svgp->pcy = ctx_y (ctx);
	break;

    case CTX_ARC: ctx_arc (ctx, svgp->numbers[0], svgp->numbers[1],
			    svgp->numbers[2], svgp->numbers[3],
			    svgp->numbers[4], svgp->numbers[5]);
        break;

    case CTX_CURVE_TO: ctx_curve_to (ctx, svgp->numbers[0], svgp->numbers[1],
					   svgp->numbers[2], svgp->numbers[3],
					   svgp->numbers[4], svgp->numbers[5]);
			svgp->pcx = svgp->numbers[2];
			svgp->pcy = svgp->numbers[3];
		        svgp->command = CTX_CURVE_TO;
        break;
    case CTX_REL_CURVE_TO:
			...
			ctx_rel_curve_to (ctx, svgp->numbers[0], svgp->numbers[1],
					   svgp->numbers[2], svgp->numbers[3],
					   svgp->numbers[4], svgp->numbers[5]);
		        svgp->command = CTX_REL_CURVE_TO;
        break;
    case CTX_LINE_TO:
        ctx_line_to (ctx, svgp->numbers[0], svgp->numbers[1]);
        svgp->command = CTX_LINE_TO;
        svgp->pcx = svgp->numbers[0];
	...
	if (svgp->n_numbers == 1)
	  ctx_rel_move_to (ctx, -svgp->numbers[0], 0.0);  //  XXX : scale by font(size)
        ...
        svgp->command = CTX_TEXT;
        break;
    case CTX_SET_FONT: ctx_set_font (ctx, (char*)svgp->holding);
        break;
...
void svgp_feed_byte (SvgP *svgp, int byte)
{
  switch (svgp->state)
  {
    case CTX_NEUTRAL:
      switch (byte)
      {
         ...
         case '{':case '}':
            break;
         case '#':
            svgp->state = CTX_COMMENT;
            break;
         case '\'':
            svgp->state = CTX_STRING1;
            svgp->pos = 0;
            svgp->holding[0] = 0;
            break;
         case '"':
            svgp->state = CTX_STRING2;
            svgp->pos = 0;
            svgp->holding[0] = 0;
            break;
         case '-':
            svgp->state = CTX_NEG_NUMBER;
            svgp->numbers[svgp->n_numbers] = 0;
            svgp->decimal = 0;
            break;
         case '0': case '1': case '2': case '3': case '4':
         case '5': case '6': case '7': case '8': case '9':
            svgp->state = CTX_NUMBER;
            svgp->numbers[svgp->n_numbers] = 0;
    ...
    case CTX_NEG_NUMBER:
      {
        ...
        switch (byte)
        {
           ...
           case 18: case 19: case 20: case 21: case 22: case 23: case 24:
           case 25: case 26: case 27: case 28: case 29: case 30: case 31:
              svgp->state = CTX_NEUTRAL;
              break;
           case ' ':case '\t':case '\r':case '\n':case ';':case ',':case '(':case ')':case '=':
           case '{':case '}':
              if (svgp->state == CTX_NEG_NUMBER)
        ...
        if ((svgp->state != CTX_NUMBER &&
             svgp->state != CTX_NEG_NUMBER) || new_neg)
        {
                 svgp->n_numbers ++;
    ...
    case CTX_STRING1:
      ...
      {
         case '\\':
            svgp->state = CTX_STRING1_ESCAPED;
            break;
         case '\'':
            svgp->state = CTX_NEUTRAL;
            break;
    ...
    case CTX_STRING2:
      ...
      {
         case '\\':
            svgp->state = CTX_STRING2_ESCAPED;
            break;
         case '"':
            svgp->state = CTX_NEUTRAL;
            break;
