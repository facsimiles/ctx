...
bool bsp_captouch_screen_touched(void) { return ct_screen_touched; }
...
static bool petal_was_down[10] = {
    false,
};
static unsigned long petal_down_time[10] = {
    0,
};
static unsigned int petal_repeats[10] = {
    0,
};
...
static int frame_done_ctx(Ctx *ctx, void *user_data) {
  ...
  if (flow3r_synthesize_key_events)
    for (int i = 0; i < 10; i++) {
      ...
      if ((down != petal_was_down[i]) ||
          (((petal_repeats[i] == 1) &&
            ((ticks - petal_down_time[i]) > first_repeat_time)) ||
           ((petal_repeats[i] > 1) &&
            ((ticks - petal_down_time[i]) > subsequent_repeat_time)))) {
        if (down) {
          if (!petal_was_down[i])
            petal_repeats[i] = 1;
          else
  ...
  if ((_app_button_state != app_button_state) ||
      ...
      ((app_repeats > 1) &&
       ((ticks - app_down_time) > subsequent_repeat_time))) {
...
Ctx *ctx_host(void) {
  ...
  ctx = ctx_new_cb(DISPLAY_WIDTH, DISPLAY_HEIGHT, CTX_FORMAT_RGB565_BYTESWAPPED,
                   ctx_set_pixels, NULL, frame_done_ctx, NULL,
...
void flash_init(void) {
  ...
  esp_err_t err =
      esp_vfs_fat_spiflash_mount_rw_wl("/flash", "vfs", &mount_config, &s_wl_handle);
  if (err != ESP_OK) {
...
static void on_captouch_data(const flow3r_bsp_captouch_state_t *st) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if ((i % 2) == 0) {
      ...
      if (pos_angular < -32767)
        pos_angular = -32767;
      else if (pos_angular > 32767)
        pos_angular = 32767;
    }
  ...
  if (best_sum < CT_DOWN_THRESHOLD) {
    ct_radial = 0;
    ct_angle = ct_angle_o = ct_angle_i = ct_quant_angle = -1000.0f;
    ;
  } else {
    ...
    if (rel_angle < -0.5 - threshold)
      rel_angle = -1.0f;
    else if (rel_angle < -0.5 + threshold)
      rel_angle = -0.5f;
    else if (rel_angle < 0.5 - threshold)
      rel_angle = 0.0f;
    else if (rel_angle < 0.5 + threshold)
...
static void audio_task(void *data) {
  while (true) {
    ...
    bool is_float =
        (ctx_client_format == CTX_F32 || ctx_client_format == CTX_F32S);
