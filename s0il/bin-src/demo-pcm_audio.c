  ...
  if (string->period < period) {
    if (string->period) {
      for (int i = string->period; i < period; i++)
        string->buffer[i] =
            string->buffer[(i - string->period) % string->period];
    }
  }
...
void string_set_target_semitone(VoiceString *string, float deadline,
                                float tone) {
  string->target_hz = semitone_to_hz(tone);
  if (string->target_hz == string->hz) {
    string->target_deadline = 0;
  } else {
    string->slide_hz_per_sample =
        ((string->target_hz - string->hz) / (deadline * sample_rate));
...
#define AUDIO_RENDERAHEAD 0.05
  ...
  if (qlen < (AUDIO_RENDERAHEAD)) {
    ...
    for (int i = 0; i < frames; i++) {
      ...
      ctx_line_to(ctx, i * 1.0 / frames * width,
                  height * 0.5 - (buf[i] / 32767.0) * height * 0.5f);
    }
