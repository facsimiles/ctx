...
tools/%: tools/%.c ctx-nofont.h 
	$(CCC) $< -o $@ -g -lm -I. -Ifonts -lpthread -Wall -lm -Ideps $(CFLAGS_warnings) -DCTX_NO_FONTS -DCTX_STB_TT=1
...
ctx.static: main.c ctx.h  build.conf Makefile $(MEDIA_HANDLERS_OBJS) $(CTX_SIMD_OBJS) itk.o ctx.o deps.o terminal/*.[ch] 
	$(CCC) main.c terminal/*.c $(MEDIA_HANDLERS_OBJS) -o $@ $(CFLAGS) $(CTX_CFLAGS) ctx.o $(CTX_SIMD_OBJS) itk.o deps.o $(LIBS) -DCTX_BABL=0 -DCTX_SDL=0 -DCTX_FB=1 -DCTX_CURL=0 -static 
	strip -s -x $@
