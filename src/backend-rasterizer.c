...
inline static int ctx_rasterizer_feed_pending_edges (CtxRasterizer *rasterizer)
{
      ...
      UPDATE_PARITY;
       ...
       if (parity)
	    ...
	    if (first - cov_max > CTX_RASTERIZER_MAX_EMPTIES)
	    {
                 if (cov_max>=cov_min)
                 {
                   apply_coverage (rasterizer, &dst[((cov_min) * bpp)/8], rasterizer_src, cov_min,
				   &coverage[cov_min], cov_max-cov_min + 1);
                   cov_min = maxx;
                   cov_max = minx;
                 }
	    }
  ...
  while (rasterizer->scanline <= scan_end)
    {
	  ...
	  int fed = !convex;
	    ...
	    if (fed){
          ...
          if (fed) ctx_rasterizer_sort_active_edges (rasterizer);
          ctx_rasterizer_generate_coverage (rasterizer, minx, maxx, coverage, is_winding, aa, fraction);
          rasterizer->scanline += scanline_increment;
