...
#endif
#if CTX_DITHER
      ctx_dither_rgba_u8 (rgba, x+i, y, dither_red_blue, dither_green);
#endif
    rgba+= 4;
    vv += ud_plus_vd;
  }
...
#if CTX_DITHER
  ctx_dither_graya_u8 ((uint8_t*)dst, x+i, y, rasterizer->format->dither_red_blue,
                      rasterizer->format->dither_green);
#endif
  dst += 2;
  x += dx;
...
static void ctx_RGBA8_image_rgba8_RGBA8_bi_fill_rect (CtxRasterizer *rasterizer, int x0, int y0, int x1, int y1, int copy)
{
  ...
  for (int x = 0; x < width; x++)
  {
    ...
    uint32_t *src0 = &blank;
    uint32_t *src1 = &blank;
    if (u >= 0 && u < bwidth && v >= 0 && v < bheight)
    {
      src0 = data + u + bwidth * v;
    }
    if (u + 1>= 0 && u + 1 < bwidth && v >= 0 && v < bheight)
    {
