...
static void text_key_press (CtxEvent *event, void *data1, void *data2)
{
  ...
  if (!strcmp (string, "space")) string = " ";
  else if (!strcmp (string, "return")) string = "\n";

   if (ui_keyboard_visible(ui))
   {
     if (!strcmp (string, "shift-space")){ ui_do (ui, "kb-hide"); return; }
     if (!strcmp (string, "escape")){ ui_do (ui, "kb-hide"); return; }
   }
   else
   {
  ...
  else if (!strcmp (string, "right")) text_right (event, data1, data2);
  else if (!strcmp (string, "backspace"))
  {
    ...
    if (text->pos < 0) text->pos = 0;
        memmove (&text->contents[text->pos],
                 &text->contents[old_cursor_pos],
  ...
  else if (ctx_utf8_strlen (string) == 1)
  {
    ...
    if ((int)strlen ((char*)text->contents) + insert_len + 1 >= text->capacity)
    {
       text->capacity += 512;
...

    {
      memmove (&text->contents[text->pos+insert_len],
               &text->contents[text->pos], strlen ((char*)&text->contents[text->pos]) + 1);
      memcpy (&text->contents[text->pos], string, insert_len);
      text->pos += insert_len;
    }
    text->dirty ++;
  }
  else printf ("unhandled key:[%s]\n", event->string);
}
...
#define UPDATE_NEIGHBOR() \
        do {\
        if (((text->cursor_x >= 0.0f) && (text->virtual_line >=0) && (text->up_pos == -1)) && \
         (virtual_line_no == (text->virtual_line-1)) && \
         ((x >= text->cursor_x)))\
        {\
          text->up_pos = byte_pos;\
        }\
        if (((text->virtual_line >=0) && (text->cursor_x >= 0.0f) && text->down_pos == -1) && \
         (virtual_line_no == (text->virtual_line+1)) && \
         (x >= text->cursor_x))\
        {\
...
#define MAYBE_DRAW_CURSOR() \
        do {if (byte_pos >= text->pos && !cursor_drawn)\
        {\
          cursor_drawn = true;\
          const char *end = &prev_word[strlen(prev_word)-(byte_pos-text->pos)];\
          float part_width = ctx_text_width (ctx, end);\
          ctx_save (ctx);\
          ctx_rgba (ctx, 1,0,0,0.5f);\
          float cursor_x= x - part_width;\
          float cursor_width = cw;\
          ctx_rectangle (ctx, cursor_x - cursor_width/2, y-font_size * 0.8, cursor_width, font_size);\
          if (text->cursor_x < 0.0f) text->cursor_x = cursor_x;\
          ctx_fill (ctx);\
          ctx_restore (ctx);\
        };}while(0)
...
#define ADD_WORD()\
   ...
   {
      float text_scroll_min = 0.0f - ctx_height (ctx) /6 - text->virtual_line * font_size;
      float text_scroll_max = 0.0f + ctx_height (ctx) /4 - text->virtual_line * font_size;
      if (ui_keyboard_visible(ui))
      {
...
MAIN(text)
{
  ...
  else
  {
    ...
    char *mime_types[]={"text/plain", ".txt",
                        "text/markdown", ".md",
                        "text/html", ".html",
