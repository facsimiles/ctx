...
const char *ui_get_location (Ui *ui)
{
  return ui->location;
}

int ui_get_frame_no (Ui *ui)
{
  return ui->frame_no;
}
...
Ui *ui_host(Ctx *ctx)
{
  ...
  {
    bool owns_ctx = false;
    if (!ctx){ ctx = ctx_new(-1,-1,NULL); owns_ctx = true; }
    def_ui = ui_new(ctx);
...
void ctx_set_pixels (Ctx *ctx, void *user_data,
                     int x0, int y0, int w, int h, void *buf)
{
  if (ctx == NULL) ctx = ctx_host();
  ...
  char eid[64];
  ctx_start_frame(ctx);
  ctx_define_texture (ctx, NULL, in_w, h, in_w *4, CTX_FORMAT_RGBA8,
                      scratch, eid);
