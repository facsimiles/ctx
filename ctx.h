    ...
    *((uint32_t*)(tsrc)) = *((uint32_t*)(src));
    ...
    for (x = 0; x < count-8; x+=8)
    {
      ...
      __m256i xcov = _mm256_set_epi8((coverage[7]*a)>>8, (coverage[7]*a)>>8, (coverage[7]*a)>>8, (coverage[7]*a)>>8,
                                     ...
                                     (coverage[5]*a)>>8, (coverage[5]*a)>>8, (coverage[5]*a)>>8, (coverage[5]*a)>>8,
                                     (coverage[4]*a)>>8, (coverage[4]*a)>>8, (coverage[4]*a)>>8, (coverage[4]*a)>>8,
                                     (coverage[3]*a)>>8, (coverage[3]*a)>>8, (coverage[3]*a)>>8, (coverage[3]*a)>>8,
                                     (coverage[2]*a)>>8, (coverage[2]*a)>>8, (coverage[2]*a)>>8, (coverage[2]*a)>>8,
                                     (coverage[1]*a)>>8, (coverage[1]*a)>>8, (coverage[1]*a)>>8, (coverage[1]*a)>>8,
...
#define odd_mask  _mm256_set1_epi32(0xFF00FF00)
        ...
        __m256i dst_odd  = dt & odd_mask;
        __m256i src_even = xs & even_mask;
        __m256i src_odd  = xs & odd_mask;
#else
        __m256i dt = xdst;
        __m256i xs = xsrc;
        __m256i dst_even = dt & even_mask;
      ...
      dst_odd  = _mm256_mullo_epi16(dst_odd,  _mm256_set1_epi16(212));
                     ...
                     CtxBlend               blend)
{
  ...
  else
  {
    ...
    while (count--)
    {
      ...
      if (cov == 255) for (int c = 0; c < components; c++)
      {
        int res = 0;
        switch (f_s)
        {
      ...
      else for (int c = 0; c < components; c++)
      {
        int res = 0;
        switch (f_s)
        {
          case CTX_PORTER_DUFF_0: break;
          case CTX_PORTER_DUFF_1:             res += (tsrc[c] * cov)/255; break;
          case CTX_PORTER_DUFF_ALPHA:         res += (tsrc[c] * cov * dst[components-1])/(255*255); break;
          case CTX_PORTER_DUFF_1_MINUS_ALPHA: res += (tsrc[c] * cov * (255-dst[components-1]))/(255*255); break;
        }
        switch (f_d)
        {
          case CTX_PORTER_DUFF_0: break;
          case CTX_PORTER_DUFF_1:             res += (dst[c]); break;
          case CTX_PORTER_DUFF_ALPHA:         res += (dst[c] * tsrc[components-1] * cov)/(255*255); break;
          case CTX_PORTER_DUFF_1_MINUS_ALPHA: res += (dst[c] * ((255*255)-tsrc[components-1] * cov))/(255*255); break;
        }
