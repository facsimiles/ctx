                     ...
                     CtxBlend               blend)
{
  ...
  if ((size_t)(dst)&31)
  {
    int pre_count = 32-(((size_t)(dst))&31);
  ...
  int post_count = 32-(count & 31);
  __ctx_avx2_porter_duff (rasterizer, components, dst, src, clip, x0, coverage, count - post_count, compositing_mode, fragment, blend);
  if (post_count)
  {
  x0 += (count - post_count);
