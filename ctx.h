...
#define x0080     _mm256_set1_epi16(0x0080)
    ...
    for (; x < count-8; x+=8)
    {
     ...
     else
     {
...
#if CTX_GRADIENT_CACHE
      if (vv < 0.0f)
        ((uint32_t*)tsrc)[i] = *((uint32_t*)(&ctx_gradient_cache_u8_a[0]));
      else if (vv > 1.0f)
        ((uint32_t*)tsrc)[i] = *((uint32_t*)(&ctx_gradient_cache_u8_a[CTX_GRADIENT_CACHE_ELEMENTS][0]));
      else
      ...
      ctx_u8_associate_alpha (components, &tsrc[i*4]);
#endif
      if (tsrc[i*4+3]!=255)
        a = 0;
#if CTX_DITHER
...
#if CTX_GRADIENT_CACHE
      if (vv < 0.0f)
        ((uint32_t*)tsrc)[0] = *((uint32_t*)(&ctx_gradient_cache_u8_a[0]));
      else if (vv > 1.0f)
        ((uint32_t*)tsrc)[0] = *((uint32_t*)(&ctx_gradient_cache_u8_a[CTX_GRADIENT_CACHE_ELEMENTS][0]));
      else
...
//    if (blend != CTX_BLEND_NORMAL)
 ...
 //         tsrc[i*components+c]=src[c];
    ...
    xsrc_a = _mm256_srli_epi32(xsrc, 24);  // XX 25 is RGB specific
    if (!is_full)
    ...
    if (!is_full)
    {
      ...
      src_hi = _mm256_mulhi_epu16(
        ...
        src_lo = 
               _mm256_mulhi_epu16 (
                   _mm256_adds_epu16 (_mm256_mullo_epi16(src_lo,
                                      _mm256_sub_epi16(x00ff,xdst_a)), x0080), x0101);
        src_hi = 
               _mm256_mulhi_epu16 (
                   _mm256_adds_epu16 (_mm256_mullo_epi16(src_hi,
                                      _mm256_sub_epi16(x00ff,xdst_a)), x0080), x0101);
      ...
      switch (f_d)
      {
        ...
        case CTX_PORTER_DUFF_ALPHA:        
            ...
            break;
        case CTX_PORTER_DUFF_1_MINUS_ALPHA:
            //res += (dst[c] * (255-tsrc[components-1]))/255;
            dst_lo = _mm256_and_si256 (xdst, lo_mask); 
            dst_hi = _mm256_srli_epi16 (_mm256_and_si256 (xdst, hi_mask), 8); 
            dst_lo = 
               _mm256_mulhi_epu16 (
                   _mm256_adds_epu16 (_mm256_mullo_epi16(dst_lo,
                                      _mm256_sub_epi16(x00ff,xsrc_a)), x0080), x0101);
            dst_hi = 
               _mm256_mulhi_epu16 (
                   _mm256_adds_epu16 (_mm256_mullo_epi16(dst_hi,
                                      _mm256_sub_epi16(x00ff,xsrc_a)), x0080), x0101);
...
#else
      dst_hi = _mm256_slli_epi16 (dst_hi, 8);
      _mm256_storeu_si256((__m256i*)dst, _mm256_blendv_epi8(dst_lo, dst_hi, hi_mask));
#endif
                     ...
                     CtxBlend               blend)
{
  ...
  __ctx_u8_porter_duff (rasterizer, components,
     dst, src, clip, x0, coverage, pre_count, compositing_mode, fragment, blend);
  ...
  ctx_avx2_porter_duff (rasterizer, components, dst, src, clip, x0, coverage, count-post_count, compositing_mode, fragment, blend);
  //__ctx_u8_porter_duff (rasterizer, components, dst, src, clip, x0, coverage, count-post_count, compositing_mode, fragment, blend);
  if (src && 0)
  {
    src[0]*=2;
    src[1]*=2;
  ...
  if (post_count > 0)
  {
       ...
       __ctx_u8_porter_duff (rasterizer, components, dst, src, clip, x0, coverage, post_count, compositing_mode, fragment, blend);
  }
#else
  __ctx_u8_porter_duff (rasterizer, components, dst, src, clip, x0, coverage, count, compositing_mode, fragment, blend);
#endif
}
