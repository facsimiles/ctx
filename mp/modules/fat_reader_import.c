...
{
	mp_reader_epicfat_t *reader = (mp_reader_epicfat_t *)data;
	mp_vfs_file_close(reader->fd);
	m_del_obj(mp_reader_epicfat_t, reader);
}

void mp_reader_new_file(mp_reader_t *reader, const char *filename)
{
	int fd = mp_vfs_file_open(filename, "r");
	if (fd < 0) {
		mp_raise_OSError(-fd);
	}
	mp_reader_epicfat_t *rp = m_new_obj(mp_reader_epicfat_t);
	rp->fd                  = fd;
	int n = mp_vfs_file_read(rp->fd, rp->buf, sizeof(rp->buf));
	if (n < 0) {
...
{
	struct mp_vfs_stat stat;

	if (mp_vfs_file_stat(path, &stat) == 0) {
